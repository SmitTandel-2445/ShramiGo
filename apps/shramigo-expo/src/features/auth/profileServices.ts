...
export interface CustomerProfile {
  ...
  bio: string | null; created_at: string; updated_at: string;
  latitude?: number | null; longitude?: number | null;
}

export interface CustomerProfileUpdateData {
  full_name?: string; phone?: string; address?: string; city?: string;
  state?: string; pincode?: string; profile_image?: string; bio?: string;
}
...
export async function updateCustomerProfile(data: CustomerProfileUpdateData): Promise<CustomerProfile> {
  return apiRequest<CustomerProfile>('/api/profile/customer', {
