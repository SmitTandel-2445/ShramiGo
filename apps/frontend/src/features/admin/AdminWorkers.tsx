...
import { getAdminWorkers } from "@/features/admin/services";
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setLoadError(
          err instanceof Error ? err.message : "Unable to load workers.",
        );
      } finally {
        setLoading(false);
      }
      ...
      <main className="max-w-7xl mx-auto px-5 sm:px-8 py-7">
        {loadError && (
          <div className="mb-6 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
            Unable to load workers: {loadError}
          </div>
        )}
        ...
        <div className="mb-7 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <p className="text-xs text-gray-500 mt-4">Total Workers</p>

            <p className="text-2xl font-bold mt-1">{workerList.length}</p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <p className="text-xs text-gray-500 mt-4">Verified</p>

            <p className="text-2xl font-bold mt-1">
              {
                workerList.filter((worker) => worker.status === "Verified")
                  .length
              }
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            <div className="w-10 h-10 rounded-xl bg-orange-50 flex items-center justify-center">
              <Filter size={20} className="text-[#FF5A00]" />
            </div>
            ...
            <p className="text-2xl font-bold mt-1">
              {
                workerList.filter((worker) => worker.status === "Pending")
                  .length
              }
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <p className="text-xs text-gray-500 mt-4">Suspended</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <div>
              <h2 className="font-bold">Registered Workers</h2>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-[#087F7A]/10 flex items-center justify-center">
                          <UserRound size={18} className="text-[#087F7A]" />
                        </div>
              ...
              <div key={worker.id} className="p-5">
                ...
                <div className="mt-4 grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm font-medium mt-1">{worker.service}</p>
                  </div>
          ...
          {filteredWorkers.length === 0 && (
            <div className="py-14 text-center">
              ...
              <p className="font-medium mt-3">No workers found</p>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center px-4">
          <div className="w-full max-w-md bg-white rounded-3xl shadow-xl p-6">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold">Worker Verification</h2>
            ...
            <div className="flex items-center gap-4 mt-6">
              <div className="w-14 h-14 rounded-full bg-[#087F7A]/10 flex items-center justify-center">
                <UserRound size={25} className="text-[#087F7A]" />
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-1 mt-1">
                  <Star size={14} className="text-[#FF5A00] fill-[#FF5A00]" />
            ...
            <div className="mt-6 space-y-4">
              ...
              <DetailRow label="Experience" value={selectedWorker.experience} />

              <DetailRow label="Location" value={selectedWorker.location} />
              ...
              <DetailRow label="Current Status" value={selectedWorker.status} />
            </div>

            {/* Verification Documents */}
            <div className="mt-6 p-4 rounded-2xl bg-gray-50">
              <div className="flex items-center gap-2">
                <ShieldCheck size={18} className="text-[#087F7A]" />

                <p className="text-sm font-semibold">Verification Documents</p>
              </div>

              <p className="text-xs text-gray-500 mt-2">
                Identity, skill and worker verification documents can be
                reviewed here.
              </p>
            </div>
